...
# PARAMETERS ######################################
CAPTURE_FILE = "../captures/jitter_15k.pcapng"
USER_AGENT = "python-requests/2.32.5" # python-requests/2.34.2 python-requests/2.32.5
...
data = i_forest_t_test.isolation_forest(data,NEW_FEATURE_LIST,contamination=0.33,experimental=False,n_estimators=1000,debug=False)
tt = round(time.time() - st,2)
print(f"Duration: {tt}s")
...
show.show(data,mc_id,debug=False)
tt = round(time.time() - st,2)
print(f"Duration: {tt}s")